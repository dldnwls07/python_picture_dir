"""
AIHelper — Gemini API 연동을 통한 일기 요약/공감 기능 처리
"""
...
class AIHelper:
    """Gemini API를 호출하여 일기를 요약하고 공감해주는 헬퍼 클래스."""
    ...
    def _request(self, prompt: str, image_base64: str, response_schema: dict, required_fields: list) -> dict:
        """Gemini API에 공통 요청을 보내고 JSON 응답을 파싱해서 반환한다."""
    ...
    def summarize_diary(self, date: str, content: str, location: str = "", weather: str = "", emotion: str = "") -> str:
        """일기 본문을 한 문장으로 요약한다.
        ...
        prompt = (
            ...
            f"일기 본문:\n{content}\n\n"
            "위 일기 본문의 핵심 내용을 1문장으로 간결하게 요약해 주세요(summary)."
        )
        ...
        schema = {
            ...
            "properties": {
                "summary": {
                    "type": "STRING",
                    "description": "일기 내용의 1문장 요약"
                }
            },
            "required": ["summary"]
        }

        result = self._request(prompt, image_base64="", response_schema=schema, required_fields=["summary"])
    ...
    def analyze_empathy(
        ...
        """일기와 그림을 분석하여 공감 멘트와 그림 분석을 JSON으로 반환한다.

        Returns:
            dict: {"empathy": "공감 멘트", "drawing_analysis": "그림 분석"}
        """
        self._ensure_api_key()
        if not content.strip():
            raise ValueError("분석할 일기 내용이 비어있습니다.")
        ...
        prompt = (
            ...
            "1. 사용자의 상황과 감정에 공감하며 격려하거나 기쁨을 나누는 위로/조언의 멘트를 3~4문장으로 작성해주세요(empathy).\n"
            ...
            "2. 첨부된 그림 일기를 분석하여 사용자의 심리 상태, 색채 표현, 선의 형태 등을 분석해 주세요(drawing_analysis). "
        ...
        schema = {
            ...
            "required": ["empathy", "drawing_analysis"]
        }
        ...
        return self._request(
            ...
            required_fields=["empathy", "drawing_analysis"]
        )
