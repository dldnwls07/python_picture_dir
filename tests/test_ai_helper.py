...
class AIHelperTest(unittest.TestCase):
    ...
    def test_missing_api_key_raises_value_error(self):
        ...
        with self.assertRaises(ValueError) as ctx:
            helper.analyze_diary("2026-07-08", "일기 내용")
    ...
    def test_empty_content_raises_value_error(self):
        ...
        helper = AIHelper()
        with self.assertRaises(ValueError) as ctx:
            helper.analyze_diary("2026-07-08", "")
    ...
    def test_successful_api_call(self, mock_post):
        ...
        mock_response.json.return_value = {
            "candidates": [{
                "content": {
                    "parts": [{
                        "text": '{"summary": "테스트 요약", "empathy": "따뜻한 공감", "drawing_analysis": "테스트 그림 분석"}'
                    }]
                }
            }]
        }
        ...
        helper = AIHelper()
        result = helper.analyze_diary("2026-07-08", "오늘 너무 슬펐어")
        
        self.assertEqual(result["summary"], "테스트 요약")
    ...
    def test_network_failure_raises_runtime_error(self, mock_post):
        ...
        with self.assertRaises(RuntimeError) as ctx:
            helper.analyze_diary("2026-07-08", "오늘의 일기")
